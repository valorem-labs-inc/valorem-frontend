...
class NewOption extends React.Component {
    ...
    handleWriteContract = async () => {
        ...
        let optionId = await this.handleGetOptionTypeId(this.contractWithSigner, this.handleGetOptionTypeHash(this.optionType));
        ...
        if (!this.state.balanceTooLow && !this.state.needsApproval) {
            if (optionId._hex === '0x00') {
                this.contractWithSigner.newChain(this.optionType).then(async (response) => {
